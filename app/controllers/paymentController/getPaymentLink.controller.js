...
/**
 * Handles the get payment link request.
 * @param {*} request - The incoming request object.
 ...
 */
async function getPaymentLink(request, response, next) {
  try {
    ...
    return response.status(200).json({
      message: API_RESPONSE.PAYMENT.GET_PAYMENT_LINK_SUCCESS,
      data: null,
      status: 200,
    });
  } catch (error) {
    logger.error("Error get payment link controller:", JSON.stringify(error));
    next(error || new Error(API_RESPONSE.PAYMENT.GET_PAYMENT_LINK_FAILURE));
  }
}
