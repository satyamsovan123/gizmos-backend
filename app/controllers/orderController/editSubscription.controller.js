...
/**
 * Handles the edit subscription request.
 * @param {*} request - The incoming request object.
 ...
 */
async function editSubscription(request, response, next) {
  try {
    ...
    return response.status(200).json({
      message: API_RESPONSE.ORDER.SUBSCRIPTION_UPDATED,
      data: null,
      status: 200,
    });
  } catch (error) {
    logger.error(
      "Error in edit subscription controller:",
      JSON.stringify(error)
    );
    next(error || new Error(API_RESPONSE.ORDER.SUBSCRIPTION_UPDATE_FAILED));
  }
}
