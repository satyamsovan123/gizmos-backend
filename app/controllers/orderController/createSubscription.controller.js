...
/**
 * Handles the create subscription request.
 * @param {*} request - The incoming request object.
...
async function createSubscription(request, response, next) {
  try {
    ...
    return response.status(200).json({
      message: API_RESPONSE.ORDER.SUBSCRIPTION_CREATED,
      data: null,
      status: 200,
    });
  } catch (error) {
    logger.error(
      "Error in create subscription controller:",
      JSON.stringify(error)
    );
    next(error || new Error(API_RESPONSE.ORDER.SUBSCRIPTION_CREATION_FAILED));
  }
}
