...
async function getOrder(request, response, next) {
  try {
    ...
    return response.status(200).json({
      message: API_RESPONSE.ORDER.ORDER_FETCH_SUCCESS,
      data: null,
      status: 200,
    });
  } catch (error) {
    logger.error("Error in get order controller:", JSON.stringify(error));
    next(error || new Error(API_RESPONSE.ORDER.ORDER_FETCH_FAILURE));
  }
}
