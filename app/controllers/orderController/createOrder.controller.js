...
/**
 * Handles the create order request.
 * @param {*} request - The incoming request object.
...
async function createOrder(request, response, next) {
  try {
    ...
    return response.status(200).json({
      message: API_RESPONSE.ORDER.ORDER_PLACED_SUCCESS,
      data: null,
      status: 200,
    });
  } catch (error) {
    logger.error("Error in create order controller:", JSON.stringify(error));
    next(error || new Error(API_RESPONSE.ORDER.ORDER_PLACED_FAILURE));
  }
}
