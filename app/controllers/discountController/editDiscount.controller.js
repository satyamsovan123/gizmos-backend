...
/**
 * Handles the edit discount request.
 * @param {*} request - The incoming request object.
 ...
 */
async function editDiscount(request, response, next) {
  try {
    ...
    return response.status(200).json({
      message: API_RESPONSE.DISCOUNT.DISCOUNT_UPDATE_SUCCESS,
      data: null,
      status: 200,
    });
  } catch (error) {
    logger.error("Error in edit discount controller:", JSON.stringify(error));
    next(error || new Error(API_RESPONSE.DISCOUNT.DISCOUNT_UPDATE_FAILURE));
  }
}
