...
/**
 * Handles the get discounts request.
 * @param {*} request - The incoming request object.
...
async function getDiscounts(request, response, next) {
  try {
    ...
    return response.status(200).json({
      message: API_RESPONSE.DISCOUNT.DISCOUNT_FETCH_SUCCESS,
      data: null,
      status: 200,
    });
  } catch (error) {
    logger.error("Error in get discounts controller: ", JSON.stringify(error));
    next(error || new Error(API_RESPONSE.DISCOUNT.DISCOUNT_FETCH_FAILURE));
  }
}
