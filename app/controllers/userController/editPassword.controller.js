...
/**
 * Handles the edit password request.
 * @param {*} request - The incoming request object.
...
async function editPassword(request, response, next) {
  try {
    ...
    return response.status(200).json({
      message: API_RESPONSE.AUTHENTICATION.PASSWORD_RESET_SUCCESS,
      data: null,
      status: 200,
    });
  } catch (error) {
    logger.error("Error in edit password controller:", JSON.stringify(error));
    next(
      error || new Error(API_RESPONSE.AUTHENTICATION.PASSWORD_RESET_FAILURE)
    );
  }
}
