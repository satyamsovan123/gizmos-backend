...
async function editUser(request, response, next) {
  try {
    ...
    return response.status(200).json({
      message: API_RESPONSE.USER.USER_PROFILE_UPDATE_SUCCESS,
      data: null,
      status: 200,
    });
  } catch (error) {
    logger.error("Error in edit user controller:", JSON.stringify(error));
    next(error || new Error(API_RESPONSE.USER.USER_PROFILE_UPDATE_FAILURE));
  }
}
