...
/**
 * Handles the edit product request.
 * @param {*} request - The incoming request object.
...
async function editProduct(request, response, next) {
  try {
    ...
    return response.status(200).json({
      message: API_RESPONSE.PRODUCT.PRODUCT_UPDATE_SUCCESS,
      data: null,
      status: 200,
    });
  } catch (error) {
    logger.error("Error in edit product controller:", JSON.stringify(error));
    next(error || new Error(API_RESPONSE.PRODUCT.PRODUCT_UPDATE_FAILURE));
  }
}
