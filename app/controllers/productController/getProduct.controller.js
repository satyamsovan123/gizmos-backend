...
/**
 * Handles the get product request.
 * @param {*} request - The incoming request object.
...
async function getProduct(request, response, next) {
  try {
    ...
    return response.status(200).json({
      message: API_RESPONSE.PRODUCT.PRODUCT_FETCH_SUCCESS,
      data: null,
      status: 200,
    });
  } catch (error) {
    logger.error("Error in get product controller:", JSON.stringify(error));
    next(error || new Error(API_RESPONSE.PRODUCT.PRODUCT_FETCH_FAILURE));
  }
}
