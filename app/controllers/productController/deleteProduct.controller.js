...
async function deleteProduct(request, response, next) {
  try {
    ...
    return response.status(200).json({
      message: API_RESPONSE.PRODUCT.PRODUCT_DELETION_SUCCESS,
      data: null,
      status: 200,
    });
  } catch (error) {
    logger.error("Error in delete product controller:", JSON.stringify(error));
    next(error || new Error(API_RESPONSE.PRODUCT.PRODUCT_DELETION_FAILURE));
  }
}
