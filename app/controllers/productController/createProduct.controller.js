...
async function createProduct(request, response, next) {
  try {
    ...
    return response.status(200).json({
      message: API_RESPONSE.PRODUCT.PRODUCT_CREATION_SUCCESS,
      data: null,
      status: 200,
    });
  } catch (error) {
    logger.error("Error in create product controller:", JSON.stringify(error));
    next(error || new Error(API_RESPONSE.PRODUCT.PRODUCT_CREATION_FAILURE));
  }
}
